...
class Abs : Fragment(), ActivityCompat.OnRequestPermissionsResultCallback {
    ...
    private var abs_reps = 0
    var ltorso_angle: Int = 0
    var rtorso_angle: Int = 0
    var torso_angle: Int = 0
    private var abs_down_flag = false
    ...
    private val stateCallback = object : CameraDevice.StateCallback() {
        ...
        override fun onOpened(cameraDevice: CameraDevice) {
            ...
            this@Abs.cameraDevice = cameraDevice
        ...
        override fun onDisconnected(cameraDevice: CameraDevice) {
            cameraOpenCloseLock.release()
            cameraDevice.close()
            this@Abs.cameraDevice = null
        }

        override fun onError(cameraDevice: CameraDevice, error: Int) {
            onDisconnected(cameraDevice)
            this@Abs.activity?.finish()
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        botonir.setOnClickListener{
            var torso_angle:Int = torso_angle-80
            var abs_reps:Int = abs_reps
            val intent = Intent(context, CameraActivity::class.java)
            intent.putExtra("torso_angle", torso_angle)
            intent.putExtra("abs_reps", abs_reps)
    ...
    private fun analyze_abs(canvas: Canvas, person: Person) {
        ...
        var abs_angle_range = 150..180
        var notabs_angle_range = 155..165


        var lshoulder_xy: List<Float> =
            find_bodypart_xy(person, 5, widthRatio, heightRatio, left, top)
        var rshoulder_xy: List<Float> =
        ...
        ltorso_angle = get_angle(lhip_xy, lshoulder_xy, lknee_xy)
        rtorso_angle = get_angle(rhip_xy, rshoulder_xy, rknee_xy)

        torso_angle = minOf(ltorso_angle, rtorso_angle)



        canvas.drawText(
            "Torso angle: %d".format(torso_angle),
            (15.0f * widthRatio),
        ...
        if (torso_angle in abs_angle_range) {
            ...
            // the moment caving abs are detected, flash on screen
        ...
        if (torso_angle in notabs_angle_range) {
            // if athlete has achieved desired abs depth
            if (abs_down_flag == true) {
                abs_down_flag = false
                abs_reps += 1
            }
        }
