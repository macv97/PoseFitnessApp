...
class PushUp :
    ...
    private var pushup_reps = 0
    private var pushup_down_flag = false
    ...
    private val stateCallback = object : CameraDevice.StateCallback() {
        ...
        override fun onOpened(cameraDevice: CameraDevice) {
            ...
            this@PushUp.cameraDevice = cameraDevice
        ...
        override fun onDisconnected(cameraDevice: CameraDevice) {
            cameraOpenCloseLock.release()
            cameraDevice.close()
            this@PushUp.cameraDevice = null
        }

        override fun onError(cameraDevice: CameraDevice, error: Int) {
            onDisconnected(cameraDevice)
            this@PushUp.activity?.finish()
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        botonir.setOnClickListener{
            var pushupreps:Int = pushup_reps
            var lh_le_ls_angle:Int = lh_le_ls_angle
            var rh_re_rs_angle:Int = rh_re_rs_angle

            val intent = Intent(context, CameraActivity::class.java)
            intent.putExtra("pushupreps", pushupreps)
            intent.putExtra("plh_le_ls_angle", lh_le_ls_angle)
            intent.putExtra("prh_re_rs_angle", rh_re_rs_angle)
    ...
    private fun analyze_plank(canvas: Canvas, person: Person){
        ...
        var arm_angle_range = 90..170
        var stand_position = 165..175
        ...
        canvas.drawText(
                "push-ups reps: %d".format(pushup_reps),
                (130.0f * widthRatio),
                (-30.0f * heightRatio + top),
                squatTextPaint
        )
        ...
        if(lh_le_ls_angle in arm_angle_range || rh_re_rs_angle in arm_angle_range)
        {
            ...
            pushup_down_flag = false
        }
        if (lh_le_ls_angle in stand_position || rh_re_rs_angle in stand_position) {
            // if athlete has achieved desired squat depth
            if (pushup_down_flag == true) {
                pushup_down_flag = false
                pushup_reps += 1
            }
        }
